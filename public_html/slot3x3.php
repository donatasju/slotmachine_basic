...
function form_success($safe_input, $form) {
    ...
    $slot3x3 = new \App\Slotmachine3x3($images_array);
    $slot3x3->spin();

    if ($slot3x3->checkWin()) {
        $player = new \App\Player('player_cookie');
        $total_balance = $player->getBalance() + (2 * \App\Slotmachine::FAIRNESS_CONF * (3 ** 3));
        $player->setBalance($total_balance);
    } else {
        $player = new \App\Player('player_cookie');
        $total_balance = $player->getBalance() - 2;
        $player->setBalance($total_balance);
    }
}
...
<html>
    <head>
        <title>Suka Casino Slot 3x3</title>
    ...
    <body class="slot3x3">
        ...
        <section class="slot3x3-content">
            <h1>3x3 Tavo Bapkes Sues</h1>

            <div class="slot3x3-images">
                <?php foreach ($slot3x3->getResult() as $images_array): ?>
